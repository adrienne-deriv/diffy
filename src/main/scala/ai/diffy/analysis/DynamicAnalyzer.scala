...
/** Filters a DifferenceAnalyzer using a specified time range to output another
  * DifferenceAnalyzer
  */
object DynamicAnalyzer {
  ...
  def objectNodeToFieldMap(objectNode: ObjectNode): FieldMap = {
    ...
    if (acc.containsKey("headers")) {
      acc.put(
        "headers",
        objectNodeToFieldMap(acc.get("headers").asInstanceOf[ObjectNode])
      )
    }
...
class DynamicAnalyzer(repository: DifferenceResultRepository) {
  def filter(start: Long, end: Long): Report = {
    ...
    val summary = SummaryDifferenceCounter(
      new InMemoryDifferenceCounter("summary")
    )

    val joinedDifferences = JoinedDifferences(raw, noise)
    val analyzer = new DifferenceAnalyzer(raw, noise, summary, collector)
    val diffs = repository.findByTimestampMsecBetween(start, end)
    diffs.forEach(dr => {
      val request = Message(Some(dr.endpoint), decodeFieldMap(dr.request))
      val primary =
        Message(Some(dr.endpoint), decodeFieldMap(dr.responses.primary))
      val secondary =
        Message(Some(dr.endpoint), decodeFieldMap(dr.responses.secondary))
      val candidate =
        Message(Some(dr.endpoint), decodeFieldMap(dr.responses.candidate))
