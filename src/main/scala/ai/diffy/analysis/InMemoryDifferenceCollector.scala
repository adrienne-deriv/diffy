...
class InMemoryDifferenceCounter(name: String) extends DifferenceCounter {
  ...
  val endpointsMap: mutable.Map[String, InMemoryEndpointMetadata] =
    mutable.Map.empty

  protected[this] def endpointCollector(ep: String) =
    endpointsMap.getOrElseUpdate(
      ep,
      new InMemoryEndpointMetadata(
        receiver.withAdditionalTags(Map("endpoint" -> ep))
      )
    )
  ...
  override def fields(ep: String): Map[String, FieldMetadata] =
    endpointCollector(ep).fields
  ...
  def countOnce(endpoint: String, summaryType: String): Unit =
    endpointCollector(endpoint).addOnce(summaryType)
}
...
class InMemoryEndpointMetadata(receiver: MetricsReceiver)
  ...
  val summaryDifferenceCounter = receiver.withNameToken("differences").counter
  val summaryTotalCounter = receiver.withNameToken("total").counter
  ...
  def getMetadata(field: String): InMemoryFieldMetadata = {
    if (!_fields.contains(field)) {
      _fields += (field -> new InMemoryFieldMetadata(
        receiver.withAdditionalTags(Map("field" -> field))
      ))
    }
    _fields(field)
  }
  ...
  def addOnce(summaryType: String): Unit = {
    if (summaryType == "total") {
      Try(summaryTotalCounter.increment())
    } else {
      Try(summaryDifferenceCounter.increment())
    }
  }
  ...
  def add(diffs: Map[String, Difference]): Unit = {
    ...
    if (
      diffs.filterNot { case (_, diff) =>
        diff.isInstanceOf[NoDifference[_]]
      }.nonEmpty
    ) {
      differenceCounter.increment()
...
object InMemoryDifferenceCollector {
  val DifferenceResultNotFoundException = new Exception(
    "Difference result not found"
  )
}
...
class InMemoryDifferenceCollector {
  ...
  private[this] def sanitizePath(p: String) =
    p.stripSuffix("/").stripPrefix("/")
  ...
  def prefix(field: Field): Iterable[DifferenceResult] =
    (fields.toSeq flatMap {
      case (Field(endpoint, path), value)
          if endpoint == field.endpoint && path.startsWith(field.prefix) =>
        value
      case _ => Nil
    }).toSeq.distinct
